...
function getSectionAndAddIn(id, color, image, alt, name, color, quantity) {
  ...
  appendToElement(
    createArticleWithAtribute(id, color, image, alt, name, color, quantity),
    getSection
  );
}

// Return un article avec la div(image) et la div(contenu)
function createArticleWithAtribute(
  id,
  color,
  image,
  alt,
...
function createDivContent(name, color, id, quantity) {
  ...
  appendToElement(
    createBlocDivDescription(name, color, id, quantity),
    divCartItemContent
  );
...
function createDivSettings(quantity) {
  ...
  appendToElement(
    createBlocDivSettingsQuantity(quantity),
    divCartItemContentSettings
  );
...
function changeQuantityInStorage() {
  let getInput = document.querySelectorAll(
  ...
  for (let j = 0; j < getInput.length; j++) {
    getInput[j].addEventListener("change", (e) => {
      ...
      let foundProduct = basket.find(
        (p) => p.id == getChangeId && p.color == getChangeColor
      );
...
function validAddress(input) {
  if (
    /^[0-9]{1,2}\s[0-9A-Za-zàâäéèêëïîôöùûüÿç-\s]{3,50}\s[0-9]{5}$/.test(
      input.value
    )
  ) {
    document.getElementById("addressErrorMsg").innerText = "Adresse valide";
    return true;
  } else {
    document.getElementById("addressErrorMsg").innerText =
      "Adresse au mauvais format veuillez essayer avec ce format : '00 rue du 8 mai 50258'";
    return false;
  }
}
...
function validEmail(input) {
  if (
    /^[0-9A-Za-z-.\w]{3,40}[@][A-Za-z0,9-\w]{1,10}.[a-z]{2,10}$/.test(
      input.value
    )
  ) {
  ...
  fetch("http://localhost:3000/api/products/order", {
    ...
    .then((value) => {
      window.location.href =
        "http://127.0.0.1:" +
        locat +
        "//front/html/confirmation.html?id=" +
        value.orderId;
    })
    .catch((err) => {
      alert(
        "Un problème est survenue nous allons vous rediriger sur la page d'acceuil"
      );
      window.location.href =
        "http://127.0.0.1:" + locat + "//front/html/docs/index.html";
    });
}
...
function clickOnButton() {
  ...
  getButton.addEventListener("click", (event) => {
    ...
    if (basket == false) {
      ...
      validEmail(getForm.email) &&
      validAddress(getForm.address) &&
      validFirstAndLastNameAndCity(getForm.city, event) &&
      validFirstAndLastNameAndCity(getForm.lastName, event) &&
      validFirstAndLastNameAndCity(getForm.firstName, event) &&
      basket != false
    ) {
      let promise = {
        contact,
